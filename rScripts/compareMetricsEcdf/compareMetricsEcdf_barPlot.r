...
figure <- ggplot(data=metrics, aes(x=measurement, y=value, group=1)) +
  stat_summary(aes(fill=measurement), geom="bar", fun.y=mean, size = 5) +
  ...
  theme_bw() +
  scale_fill_manual(name=parameterName,
                    values=colorRampPalette(c("blue", "red"))(length(unique(metrics$measurement)))) +
...
# save figure as rdata
save(figure, file=paste(outFilePath, ".RData", sep=""))
# save figure as pdf
