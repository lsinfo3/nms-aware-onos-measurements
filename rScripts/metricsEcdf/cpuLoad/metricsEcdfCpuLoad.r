...
figure1 <- ggplot(data=metrics[metrics[["variable"]]=='CPU Load', ], aes(x=value, color=parameter)) +
  stat_ecdf(geom="step", na.rm=TRUE) +
  coord_cartesian(xlim=c(0.0, 1.0)) +
  labs(x="CPU Load", y="Cumulative Probability") +
  theme_bw() +
  scale_color_manual(name=legendTitle, labels=labels, values=colorRampPalette(c("blue", "red"))(5)) +
...
figure2 <- ggplot(data=metrics[metrics[["variable"]]=="CPU Load", ], aes(x=parameter, y=value, group=1)) +
...
figure3 <- ggplot(data=metrics[metrics[["variable"]]=="CPU Load", ], aes(x=parameter, y=value, fill=parameter, group=1)) +
