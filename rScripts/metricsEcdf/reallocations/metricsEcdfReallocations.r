...
figure2 <- ggplot(data=metrics[metrics[["variable"]]=="Reallocations", ], aes(x=parameter, y=value, group=1)) +
  stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
               fun.args=list(conf.int=0.95), fill="lightblue")+
  stat_summary(geom="line", fun.y=mean, linetype="dashed")+
  stat_summary(geom="point", fun.y=mean, color="red") +
  labs(x=paste(parameterName, " [s]", sep=""), y="Reallocations") +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1), text = element_text(size=12),
...
figure3 <- ggplot(data=metrics[metrics[["variable"]]=="Reallocations", ], aes(x=parameter, y=value, fill=parameter, group=1)) +
